...
fn bench_alloc(c: &mut Criterion) {
  ...
  bench_alloc_impl_!(
    ...
    objectpool::Pool::<Vec<u8>>::bounded(32, || Vec::with_capacity(4096), |_| {}),
    4
  );
  ...
  bench_alloc_impl_!(
    ...
    objectpool::Pool::<Vec<u8>>::unbounded(|| Vec::with_capacity(4096), |_| {}),
    4
  );
...
fn bench_free(c: &mut Criterion) {
  ...
  bench_free_impl_!(
    group,
    "crate 'objectpool' bounded",
    objectpool::Pool::bounded(32, || Vec::<u8>::with_capacity(16 * 1024), |_| {}),
    4
  );
  ...
  bench_free_impl_!(
    group,
    "crate 'objectpool' unbounded",
    objectpool::Pool::unbounded(|| Vec::<u8>::with_capacity(16 * 1024), |_| {}),
    4
  );
...
fn bench_reuse(c: &mut Criterion) {
  ...
  group.bench_function("crate 'objectpool' bounded", |b| {
    b.iter_batched(
      || {
        let pool =
          objectpool::Pool::bounded(VEC_SIZE, || Vec::<u8>::with_capacity(16 * 1024), |_| {});
  ...
  group.bench_function("crate 'objectpool' unbounded", |b| {
    b.iter_batched(
      || {
        let pool = objectpool::Pool::unbounded(|| Vec::<u8>::with_capacity(16 * 1024), |_| {});
        let v: Vec<_> = (0..VEC_SIZE).map(|_| pool.get()).collect();
        drop(v);
        (pool, Vec::with_capacity(VEC_SIZE))
      },
...
fn bench_forward_multi_thread(c: &mut Criterion, nb_writter: usize, nb_readder: usize) {
  ...
  bench_forward_impl_!(
    group,
    "crate 'objectpool' bounded",
    objectpool::Pool::<Vec<u8>>::bounded(32, || Vec::with_capacity(16 * 1024), |_v| {}),
    nb_readder,
    nb_writter,
    4
  );
  ...
  bench_forward_impl_!(
    group,
    "crate 'objectpool' unbounded",
    objectpool::Pool::<Vec<u8>>::unbounded(|| Vec::with_capacity(16 * 1024), |_v| {}),
    nb_readder,
    nb_writter,
    4
  );
